...
public class Worker
{
	...
	protected Position[] oldPos = new Position[3];
	protected int deadLock = 0;
    ...
    public int bouge() // SEULE FONCTION A COMPLETER !
	{
		...
		{	
			if (terrain.isColonyAtPoint(currentPos)) {
				...
				for (int i = 0; i < 10; i++) dropFoodPheromone();
		...
		;
		if (oldPos[2] != null && oldPos[1] != null) {
			if (oldPos[2].equals(currentPos) || oldPos[1].equals(currentPos)) {
				deadLock = 5;
				System.out.println(oldPos[2] + " " + currentPos);
			} else {
				deadLock = 0;
			}
		}
		...
		if (oldPos[1] != null)
			oldPos[2] = new Position(oldPos[1].x, oldPos[1].y);
		if (oldPos[0] != null)
			oldPos[1] = new Position(oldPos[0].x, oldPos[0].y);
		oldPos[0] = currentPos;
